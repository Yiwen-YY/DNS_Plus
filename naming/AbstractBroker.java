...
public abstract class AbstractBroker {
    ...
    public abstract List<Map.Entry<Subscription, Integer>> matchPublication(Publication p);
    ...
    public Integer cover(Subscription s1, Subscription s2) {
        ...
        // BigInteger bm = cut(s2, s2.getMatchValue());
        // BigInteger b1m = cut(s2, s2.getMatchValuePlusOne());
        return compare(bc, bm, b1m);
    }
    
    public BigInteger cut(Subscription s, BigInteger bm) {
        String bmStr = bm.toString();
        int cutLen = bmStr.length() / s.getServiceName().length();
        String bmCut = bmStr.substring(0, cutLen);
        return new BigInteger(bmCut);
    }
